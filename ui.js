...
async function loadBlockchainPage() {
  ...
  if (!container) return;
  try {
    const resp = await fetch('https://alethea-miner.onrender.com/supply');
    if (resp.ok) {
...
async function initValidatorPanel() {
  ...
  // Wake up the miner
fetch('https://alethea-miner.onrender.com/health', { method: 'GET' })
...
async function _approveTx(btn) {
  ...
  try {
    const resp = await fetch('https://alethea-miner.onrender.com/mine', {
